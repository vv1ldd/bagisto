...
Route::group(['domain' => config('app.admin_domain')], function () {
    ...
    /**
     * If there's an admin prefix and we are on the admin domain, 
     * ensure the root of the domain also redirects to the login.
     */
    if (config('app.admin_domain') && config('app.admin_url')) {
        Route::get('/', [Controller::class, 'redirectToLogin']);
    }
});
