...
class BlockchainSyncService
{
    ...
    public function verifyOwnership(CryptoAddress $cryptoAddress): bool
    {
        try {
            ...
            $isVerified = match ($cryptoAddress->network) {
                ...
                'arbitrum_one' => true, // Auto-verified if generated by us
                default => false,
            };
    ...
    public function syncDeposits(CryptoAddress $cryptoAddress): array
    {
        ...
        try {
            $transactions = match ($cryptoAddress->network) {
                ...
                'arbitrum_one' => $this->fetchArbitrumTransactions($cryptoAddress->address),
                'usdt_arbitrum_one' => $this->fetchUsdtArbitrumTransactions($cryptoAddress->address),
                default => [],
            };
    ...
    /**
     * Fetch Arbitrum One ETH balance.
     */
    protected function fetchArbitrumBalance(string $address): ?float
    {
    ...
    /**
     * Fetch Arbitrum One transactions (ETH).
     */
    protected function fetchArbitrumTransactions(string $address): array
    {
        $response = Http::get("https://api.arbiscan.io/api", [
            'module' => 'account',
            'action' => 'txlist',
    ...
    /**
     * Fetch Arbitrum One USDT transactions.
     */
    protected function fetchUsdtArbitrumTransactions(string $address): array
    {
        $response = Http::get("https://api.arbiscan.io/api", [
            'module' => 'account',
            'action' => 'tokentx',
            ...
            if (isset($data['result']) && is_array($data['result'])) {
                foreach ($data['result'] as $tx) {
                    $results[] = [
                        'tx_id' => $tx['hash'],
                        'to' => $tx['to'] ?? '',
                        'amount' => (float) ($tx['value'] / 10 ** 6), // USDT usually 6 decimals
                        'symbol' => 'USDT',
                    ];
                }
            }
