...
<v-mini-cart>
    <span class="icon-cart cursor-pointer text-2xl" role="button"
...
<script type="text/x-template" id="v-mini-cart-template">
        ...
        @if (core()->getConfigData('sales.checkout.mini_cart.display_mini_cart'))
            <x-shop::drawer>
                ...
                <x-slot:header>
                    ...
                    <p class="text-base max-md:text-zinc-500 max-sm:text-xs">
                        @lang(core()->getConfigData('sales.checkout.mini_cart.offer_info'))
                    </p>
                    ...
                    >
                            ...
                            <div class="">
                                <a :href="`{{ route('shop.product_or_category.index', '') }}/${item.product_url_key}`">
                                    <img
                                        ...
                                        class="max-w-28 max-h-28  max-md:max-h-20 max-md:max-w-[76px]"
                                    />
                                </a>
                            </div>
                        ...
                        <div class="grid flex-1 place-content-start justify-stretch gap-y-2.5">
                            <div class="flex justify-between gap-2 max-md:gap-0 max-sm:flex-wrap">
                                ...
                                >
                                        <p class="text-base font-bold dark:text-white max-md:font-normal max-sm:text-sm">
                                            @{{ item.name }}
                                        </p>
                                    </a>
                                    ...
                                    <template v-if="displayTax.prices == 'including_tax'">
                                        <p class="text-lg font-black dark:text-white max-md:font-semibold max-sm:text-sm">
                                ...
                                <x-shop::quantity-changer
                                    ...
                                    class="max-h-9 max-w-[150px] gap-x-2.5  px-3.5 py-1.5 max-md:gap-x-2 max-md:px-1 max-md:py-0.5"
                                    name="quantity"
                                    ::value="item?.quantity"
                                ...
                                <button
                                    type="button"
                                    class="text-[#7C45F5] font-bold uppercase tracking-widest text-[10px] max-md:text-sm"
            ...
            <x-slot:footer>
                ...
                >
                    <div
                        class="my-8 flex items-center justify-between border-b border-zinc-200 dark:border-white/5 px-6 pb-2 max-md:my-0 max-md:border-t max-md:px-5 max-md:py-2"
                    ...
                    >
                        ...
                        <template v-if="displayTax.subtotal == 'including_tax'">
                            <p class="text-3xl font-black max-md:text-base dark:text-white">
                                @{{ cart.formatted_sub_total_incl_tax }}
                            </p>
                        </template>

                        <template v-else-if="displayTax.subtotal == 'both'">
                            <p class="flex flex-col text-3xl font-black max-md:text-sm max-sm:text-right dark:text-white">
                                @{{ cart.formatted_sub_total_incl_tax }}

                                <span class="text-sm font-normal text-zinc-500 max-sm:text-xs">
                                    @lang('shop::app.checkout.cart.mini-cart.excl-tax')

                                    <span class="font-medium text-zinc-900 dark:text-zinc-300">@{{ cart.formatted_sub_total }}</span>
                                </span>
                            </p>
                        </template>
            ...
            <a href="{{ route('shop.checkout.onepage.index') }}">
                {!! view_render_event('bagisto.shop.checkout.mini-cart.drawer.toggle.before') !!}
                    ...
                    <span class="relative">
                        ...
                        <span
                            class="absolute -top-4  bg-navyBlue px-2 py-1.5 text-xs font-semibold leading-[9px] text-white ltr:left-5 rtl:right-5 max-md:px-2 max-md:py-1.5 max-md:ltr:left-4 max-md:rtl:right-4"
                            v-if="cart?.items_qty"
                        >
                            @{{ cart.items_qty }}
                        </span>
