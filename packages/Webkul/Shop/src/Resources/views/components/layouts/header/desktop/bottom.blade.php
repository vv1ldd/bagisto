...
<div
    class="flex h-[72px] w-full items-center justify-between px-[60px] max-1180:px-8 max-sm:px-4 mx-auto max-w-7xl gap-4">
        ...
        @if ($showCategories)
            <v-desktop-category>
                <div class="flex items-center gap-5">
                    <span class="w-20 h-6  shimmer" role="presentation"></span>
                    <span class="w-20 h-6  shimmer" role="presentation"></span>
                    <span class="w-20 h-6  shimmer" role="presentation"></span>
                </div>
            </v-desktop-category>
    ...
    <div id="header-toolbar-teleport-target"
        class="flex-1 flex justify-center items-center min-w-0 px-2 overflow-hidden"></div>
                        ...
                        <div class="relative px-6 pb-2">
                            <form action="{{ route('shop.search.index') }}" class="relative group">
                                ...
                                <input type="text" name="query" value="{{ request('query') }}" placeholder="Поиск..."
                                    class="w-full  border border-zinc-200 bg-zinc-50 py-2.5 pl-10 pr-4 text-sm font-medium text-zinc-700 transition-all focus:border-[#7C45F5] focus:bg-white focus:outline-none shadow-sm"
    ...
    <script type="module">
        app.component('v-desktop-category', {
            ...
            methods: {
                ...
                getCategories() {
                    this.$axios.get("{{ route('shop.api.categories.tree', ['show_in_header' => 1, 'show_in_push_menu' => 0]) }}")
                        .then(response => {
                            this.isLoading = false;
