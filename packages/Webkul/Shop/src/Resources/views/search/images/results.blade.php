<v-image-search-result-component>
    <div class="mt-8  border border-gray-200 p-5 max-sm:grid max-sm:gap-y-2.5 max-sm:p-2.5">
        <div class="flex gap-12 max-sm:items-center max-sm:gap-4">
            <x-shop::media.images.lazy class="h-[110px] max-h-[110px] w-[110px] min-w-[110px] max-w-[110px]  max-sm:h-[60px] max-sm:max-h-[60px] max-sm:min-h-[60px] max-sm:w-[60px] max-sm:min-w-[60px] max-sm:max-w-[60px] max-sm:" />
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-wrap gap-2 max-sm:hidden">
                    @for ($i = 1; $i < 10; $i++)
                        <div class="shimmer m-1 flex cursor-pointer items-center justify-center  font-medium">
                            <span class="shimmer h-10 w-20 "></span>
                        </div>
                    @endfor
                </div>
            </div>
        ...
        <div class="hidden flex-wrap gap-2 max-sm:flex">
            @for ($i = 1; $i < 8; $i++)
                <div class="shimmer m-1 flex cursor-pointer items-center justify-center  font-medium">
                    <span class="shimmer h-9 w-20 "></span>
                </div>
            @endfor
        </div>
    </div>
...
@pushOnce('scripts')
    ...
    >
        <div class="mt-8  border border-gray-200 bg-gray-100 p-5 max-sm:mt-0 max-sm:grid max-sm:gap-y-2.5 max-sm:p-2.5">
            <div class="flex gap-12 max-sm:items-center max-sm:gap-4">
                <img
                    class="h-[110px] max-h-[110px] min-h-[110px] w-[110px] max-w-[110px]  max-sm:h-[60px] max-sm:max-h-[60px] max-sm:min-h-[60px] max-sm:w-[60px] max-sm:max-w-[60px] max-sm:"
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex flex-wrap gap-5 max-sm:hidden">
                        <span 
                            class="flex cursor-pointer items-center justify-center  border border-navyBlue px-4 py-1.5 font-medium text-navyBlue"
                            :class="{' bg-navyBlue text-white': term.name.trim() === queryParameter.trim()}"
            ...
            <div class="hidden flex-wrap gap-2 max-sm:flex">
                <span 
                    class="flex cursor-pointer items-center justify-center  border border-navyBlue bg-white font-medium text-navyBlue"
                    v-for="term in searchedTerms"
                    @click="search(term)"
                >
                    <span
