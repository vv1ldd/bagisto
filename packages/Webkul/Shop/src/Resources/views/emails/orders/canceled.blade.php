        ...
        <div style="padding: 20px; background-color: #F0EFFF; border: 3px solid #18181B; margin-bottom: 34px;">
            <p style="font-size: 16px; color: #18181B; line-height: 24px; margin: 0;">
                {!! trans('shop::app.emails.orders.canceled.greeting', [
                    'order_id' => '<a href="' . route('shop.customers.account.orders.view', $order->id) . '" style="color: #7C45F5; font-weight: 900; text-underline-offset: 4px;">#' . $order->increment_id . '</a>',
                    ...
                    <td style="vertical-align: top; width: 50%; padding-right: 20px;">
                        <div style="padding: 24px; background-color: #FFFFFF; border: 3px solid #18181B; box-shadow: 8px 8px 0px 0px #F0EFFF; min-height: 350px;">
                            ...
                            <div style="font-size: 15px; color: #18181B; line-height: 25px;">
                                <span style="font-weight: 700;">{{ $order->shipping_address->company_name ?? '' }}</span><br/>
                ...
                @if ($order->billing_address)
                    <td style="vertical-align: top; width: 50%;">
                        <div style="padding: 24px; background-color: #FFFFFF; border: 3px solid #18181B; box-shadow: 8px 8px 0px 0px #F0EFFF; min-height: 350px;">
                            ...
                            <div style="font-size: 15px; color: #18181B; line-height: 25px;">
                                <span style="font-weight: 700;">{{ $order->billing_address->company_name ?? '' }}</span><br/>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr style="background-color: #F0EFFF; border-bottom: 3px solid #18181B;">
                    <th style="text-align: left; padding: 16px; font-weight: 900; text-transform: uppercase; font-size: 13px; color: #18181B;">@lang('shop::app.emails.orders.sku')</th>
                    <th style="text-align: left; padding: 16px; font-weight: 900; text-transform: uppercase; font-size: 13px; color: #18181B;">@lang('shop::app.emails.orders.name')</th>
                    <th style="text-align: left; padding: 16px; font-weight: 900; text-transform: uppercase; font-size: 13px; color: #18181B;">@lang('shop::app.emails.orders.price')</th>
                    <th style="text-align: left; padding: 16px; font-weight: 900; text-transform: uppercase; font-size: 13px; color: #18181B;">@lang('shop::app.emails.orders.qty')</th>
                </tr>
            </thead>
            ...
            <tbody>
                @foreach ($order->items as $item)
                    <tr style="border-bottom: 1px solid #18181B;">
                        ...
                        <td style="padding: 16px; font-size: 14px; color: #18181B; font-weight: 700;">{{ $item->name }}</td>
                        <td style="padding: 16px; font-size: 14px; color: #18181B;">
                            ...
                            @elseif (core()->getConfigData('sales.taxes.sales.display_prices') == 'both')
                                {{ core()->formatPrice($item->price_incl_tax, $order->order_currency_code) }}
                                <br><span style="font-size: 11px; opacity: 0.7;">@lang('shop::app.emails.orders.excl-tax'): {{ core()->formatPrice($item->price, $order->order_currency_code) }}</span>
                            @else
                                {{ core()->formatPrice($item->price, $order->order_currency_code) }}
                            @endif
                        </td>
                        <td style="padding: 16px; font-size: 14px; color: #18181B; font-weight: 900;">{{ $item->qty_canceled }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
                        ...
                        <table style="width: 100%; font-size: 15px; color: #18181B; line-height: 30px;">
                            @if (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'including_tax')
                                <tr>
                                    ...
                                    <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">{{ core()->formatPrice($order->sub_total, $order->order_currency_code_incl_tax) }}</td>
                                </tr>
                            @elseif (core()->getConfigData('sales.taxes.sales.display_subtotal') == 'both')
                                <tr>
                                    <td>@lang('shop::app.emails.orders.subtotal-excl-tax')</td>
                                    <td style="text-align: right; font-weight: 700;">{{ core()->formatPrice($order->sub_total, $order->order_currency_code) }}</td>
                                </tr>
                                <tr>
                            ...
                            @else
                                <tr>
                                    <td style="padding-bottom: 8px;">@lang('shop::app.emails.orders.subtotal')</td>
                                    <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">{{ core()->formatPrice($order->sub_total, $order->order_currency_code) }}</td>
                                </tr>
                            @endif
                            ...
                            @if ($order->shipping_address)
                                @if (core()->getConfigData('sales.taxes.sales.display_shipping_amount') == 'including_tax')
                                    <tr>
                                        ...
                                        <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">{{ core()->formatPrice($order->shipping_amount_incl_tax, $order->order_currency_code) }}</td>
                                    </tr>
                                @elseif (core()->getConfigData('sales.taxes.sales.display_shipping_amount') == 'both')
                                    <tr>
                                        <td>@lang('shop::app.emails.orders.shipping-handling-excl-tax')</td>
                                        <td style="text-align: right; font-weight: 700;">{{ core()->formatPrice($order->shipping_amount, $order->order_currency_code) }}</td>
                                    </tr>
                                    <tr>
                                ...
                                @else
                                    <tr>
                                        <td style="padding-bottom: 8px;">@lang('shop::app.emails.orders.shipping-handling')</td>
                                        <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">{{ core()->formatPrice($order->shipping_amount, $order->order_currency_code) }}</td>
                                    </tr>
                                @endif
                            @endif
                            ...
                            <tr>
                                ...
                                <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">{{ core()->formatPrice($order->tax_amount, $order->order_currency_code) }}</td>
                            </tr>

                            @if ($order->discount_amount > 0)
                                <tr>
                                    <td style="padding-bottom: 8px;">@lang('shop::app.emails.orders.discount')</td>
                                    <td style="text-align: right; padding-bottom: 8px; font-weight: 700;">-{{ core()->formatPrice($order->discount_amount, $order->order_currency_code) }}</td>
                                </tr>
                            @endif
                                    ...
                                    <div style="background-color: #7C45F5; color: #FFFFFF; padding: 16px; border: 3px solid #18181B; box-shadow: 4px 4px 0px 0px #18181B;">
                                        <table style="width: 100%; border-collapse: collapse;">
                                            <tr>
                                                ...
                                                <td style="text-align: right; font-weight: 900; font-size: 20px;">{{ core()->formatPrice($order->grand_total, $order->order_currency_code) }}</td>
                                            </tr>
                                        </table>
                                    </div>
                                </td>
