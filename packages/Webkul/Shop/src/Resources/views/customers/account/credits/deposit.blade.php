    ...
    <div class="max-w-lg mx-auto px-4 py-6">
        ...
        @else
            ...
            <div id="step-assets" class="hidden">
                <div class="mb-4">
                    <button onclick="backToNetworks()" class="text-[13px] text-zinc-400 font-medium hover:text-zinc-600 transition-colors">← Назад к выбору сети</button>
                </div>
                    ...
                    <div id="assets-group-{{ $gKey }}" class="hidden group-assets-list flex flex-col gap-3">
                        @foreach($group['assets'] as $aKey => $asset)
                            <button onclick="selectAsset('{{ $gKey }}', '{{ $aKey }}')"
                                class="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden flex items-center gap-4 px-4 py-4 hover:border-violet-200 transition-all active:scale-[0.98]">
                                <span class="w-10 h-10 bg-zinc-100 rounded-xl flex items-center justify-center text-zinc-600 text-[18px] font-bold shrink-0">{{ $asset['icon'] }}</span>
                                <div class="text-left flex-1">
                    ...
                    @foreach($group['assets'] as $aKey => $asset)
                        <div id="details-asset-{{ $aKey }}" class="hidden asset-details-view">
                            ...
                            <div class="flex items-center gap-3 mb-6">
                                <span class="w-12 h-12 rounded-2xl flex items-center justify-center text-white text-[22px] font-bold shrink-0"
                            ...
                            <div class="bg-amber-50 border border-amber-100 rounded-2xl px-4 py-4 mb-6">
                                ...
                                <p class="text-[12px] text-amber-700 leading-relaxed">
                                    Для зачисления необходимо отправить <strong>точную сумму верификации</strong> (с дробным хвостом). 
                            ...
                            <div class="bg-zinc-900 rounded-2xl p-5 shadow-inner mb-6">
                                <div class="text-[11px] text-zinc-500 uppercase font-bold tracking-widest mb-2">Адрес для пополнения ({{ $asset['symbol'] }})</div>
                                <div class="text-[15px] font-mono text-white break-words mb-4 select-all text-center leading-relaxed">
                                    {{ $asset['address'] }}
                                </div>
