...
<div class="nav-grid">
    {{-- Email Step (Onboarding only) --}}
    @if ($isOnboarding)
        @if (!$customer->email)
            <a href="{{ route('shop.customers.account.onboarding.add_email') }}" class="nav-tile group mb-1">
                <span class="w-12 h-12 flex items-center justify-center bg-orange-500 text-white rounded-2xl shrink-0 transition-transform group-hover:scale-105 shadow-sm">
        ...
        @elseif (!$customer->is_verified)
            <a href="{{ route('shop.customers.account.onboarding.verify_email_view') }}" class="nav-tile group mb-1">
                <span class="w-12 h-12 flex items-center justify-center bg-orange-400 text-white rounded-2xl shrink-0 transition-transform group-hover:scale-105 shadow-sm">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="flex flex-col min-w-0 pr-4">
                    <div class="flex items-center gap-2">
                        <span class="nav-label">Подтвердить почту</span>
                        <span class="bg-orange-100 text-orange-600 text-[10px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider">код отправлен</span>
                    </div>
                    <span class="text-[12px] text-zinc-500 font-medium truncate">{{ $customer->email }}</span>
                </div>
                <span class="nav-arrow">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"/>
