...
<div class="{{ $isOnboarding ? 'space-y-4' : 'space-y-6' }}">
    {{-- Seed Phrase --}}
    @php
    ...
    @if (!$isVerified || $needsUpgrade)
        <a href="{{ route('shop.customers.account.profile.generate_recovery_key') }}" 
            class="group relative block w-full bg-white border-4 border-zinc-900 {{ $isOnboarding ? 'p-4' : 'p-6 md:p-8' }} transition-all hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(24,24,27,1)] active:translate-x-0 active:translate-y-0 active:shadow-none shadow-[8px_8px_0px_0px_rgba(255,77,109,1)]">
            <div class="flex items-start {{ $isOnboarding ? 'gap-4' : 'gap-6' }}">
                <div class="{{ $isOnboarding ? 'w-12 h-12' : 'w-16 h-16' }} flex items-center justify-center {{ $needsUpgrade ? 'bg-amber-400' : 'bg-[#FF4D6D]' }} border-3 border-zinc-900 shadow-[4px_4px_0px_0px_rgba(24,24,27,1)] shrink-0 transition-transform group-hover:rotate-3">
                    <svg class="{{ $isOnboarding ? 'w-6 h-6' : 'w-8 h-8' }} text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                    </svg>
                </div>
                <div class="flex-1 min-w-0 pt-1">
                    <div class="flex flex-wrap items-center gap-3 {{ $isOnboarding ? 'mb-1' : 'mb-2' }}">
                        <span class="text-zinc-900 {{ $isOnboarding ? 'text-lg' : 'text-xl' }} font-black uppercase tracking-tight">Фразы восстановления</span>
                        @if ($needsUpgrade)
    ...
    <a href="{{ route('shop.customers.account.passkeys.index', ['onboarding' => $isOnboarding]) }}" 
        class="group relative block w-full bg-white border-4 border-zinc-900 {{ $isOnboarding ? 'p-4' : 'p-6 md:p-8' }} transition-all hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(24,24,27,1)] active:translate-x-0 active:translate-y-0 active:shadow-none shadow-[8px_8px_0px_0px_rgba(124,69,245,1)]">
        <div class="flex items-start {{ $isOnboarding ? 'gap-4' : 'gap-6' }}">
            <div class="{{ $isOnboarding ? 'w-12 h-12' : 'w-16 h-16' }} flex items-center justify-center bg-[#7C45F5] border-3 border-zinc-900 shadow-[4px_4px_0px_0px_rgba(24,24,27,1)] shrink-0 transition-transform group-hover:-rotate-3 text-white">
                <svg class="{{ $isOnboarding ? 'w-6 h-6' : 'w-8 h-8' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                </svg>
            </div>
            <div class="flex-1 min-w-0 pt-1">
                <div class="flex flex-wrap items-center gap-3 {{ $isOnboarding ? 'mb-1' : 'mb-2' }}">
                    <span class="text-zinc-900 {{ $isOnboarding ? 'text-lg' : 'text-xl' }} font-black uppercase tracking-tight">Устройства (Passkeys)</span>
                    @if($passkeyCount > 0)
                        <span class="bg-[#7C45F5] border-2 border-zinc-900 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-white shadow-[2px_2px_0px_0px_rgba(24,24,27,1)]">
