<x-shop::layouts.account :is-cardless="true" :title="'Видеовстреча'" :back-link="route('shop.customers.account.index')">
    <div class="mt-0 mb-6 w-full max-w-[800px] mx-auto px-1 leading-none">
        ...
        >
            <!-- Fallback for no-JS or pre-mount (Light Theme) -->
            <div class="p-6 md:p-10 bg-white rounded-[2rem] border border-[#e2d9ff] shadow-sm relative overflow-hidden group">
                <div class="relative z-10 text-center">
                    <div class="flex flex-col items-center mb-6">
                        <div class="w-16 h-16 bg-[#7C45F5]/5 text-[#7C45F5] rounded-3xl flex items-center justify-center mb-4 transition-transform group-hover:scale-105">
                        ...
                        <h3 class="text-[20px] md:text-[24px] font-black text-[#1a0050] uppercase tracking-tighter italic">Создать встречу</h3>
                    ...
                    <form action="{{ route('shop.call.store') }}" method="POST" class="flex flex-col gap-4 max-w-[400px] mx-auto w-full">
                        ...
                        <div class="w-full">
                            <input type="text" name="recipient_emails[]" placeholder="email@example.com или @alias" required
                                class="w-full bg-zinc-50 border border-zinc-100 rounded-2xl px-6 py-4 text-[15px] text-[#1a0050] focus:outline-none focus:border-[#7C45F5] transition-all placeholder:text-zinc-400 font-medium">
                        </div>
        ...
        <div class="mt-4 p-4 bg-zinc-50/50 border border-zinc-100 text-[12px] text-zinc-500 leading-tight rounded-[2rem] text-center max-w-[600px] mx-auto">
            <p><strong class="text-zinc-900 font-bold">Безопасность:</strong> Все видеовстречи осуществляются напрямую между устройствами и не записываются на сервере. Соединение защищено сквозным E2E шифрованием по умолчанию.</p>
        </div>
    </div>
