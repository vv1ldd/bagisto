<x-shop::layouts.account :is-cardless="true" :title="'Видеовстреча'" :back-link="route('shop.customers.account.index')">
    <div class="mt-2 mb-10 w-full max-w-[800px] mx-auto px-1">
        <v-meeting-inviter 
            ...
            <div class="p-10 bg-white rounded-[2rem] border border-[#e2d9ff] shadow-sm relative overflow-hidden group">
                <div class="relative z-10 text-center">
                    ...
                    <form action="{{ route('shop.call.store') }}" method="POST" class="flex flex-col gap-4 max-w-[400px] mx-auto w-full">
                        ...
                        <button type="submit" 
                            class="w-full bg-[#7C45F5] hover:bg-[#6b35e4] text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-[13px] transition-all shadow-xl shadow-[#7C45F5]/20 active:scale-[0.98] whitespace-nowrap">
                            Начать встречу
                        </button>
                    </form>
                </div>
        ...
        <div class="mt-8 p-8 bg-zinc-50/50 border border-zinc-100 text-[13px] text-zinc-500 leading-relaxed rounded-[2.5rem] text-center max-w-[600px] mx-auto">
