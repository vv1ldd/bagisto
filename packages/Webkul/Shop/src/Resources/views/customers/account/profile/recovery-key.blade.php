        ...
        <script type="text/x-template" id="seed-phrase-reveal-template">
            <div class="space-y-8">
                ...
                <div class="flex flex-col gap-6">
                    <!-- Step 1 Word Grid -->
                    <div v-if="step === 1" class="grid grid-cols-3 gap-3 w-full animate-in fade-in zoom-in-95 duration-500">
                        <div v-for="(word, index) in firstHalf" :key="index" 
                            class="flex items-center gap-2 bg-white border-2 border-zinc-900 rounded-xl p-3 shadow-[3px_3px_0px_0px_rgba(24,24,27,1)] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none group">
                            <span class="text-[10px] font-black text-zinc-300 w-5 select-none">@{{ index + 1 }}</span>
                    ...
                    <div v-if="step === 1" class="flex justify-center pt-4">
                    ...
                    <!-- Step 2 Word Grid -->
                    <div v-if="step === 2" class="grid grid-cols-3 gap-3 w-full animate-in fade-in zoom-in-95 duration-500">
                        <div v-for="(word, index) in secondHalf" :key="index" 
                            class="flex items-center gap-2 bg-zinc-50 border-2 border-dashed border-zinc-900 rounded-xl p-3 shadow-[3px_3px_0px_0px_rgba(24,24,27,1)] transition-all hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none group">
                            <span class="text-[10px] font-black text-zinc-300 w-5 select-none">@{{ splitIndex + index + 1 }}</span>
                            <span class="text-zinc-900 font-black tracking-tight text-xs select-all truncate lowercase">@{{ word }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="step === 2" class="flex flex-col items-center gap-8 pt-4 animate-in fade-in slide-in-from-top-4 duration-700 delay-200">
                    ...
                    <p class="text-zinc-500 text-[9px] font-black uppercase tracking-widest text-center max-w-[320px] leading-relaxed">
